...
export async function POST(request: NextRequest) {
  ...
  const payload = {
    ...
    customer_email:
      typeof body?.customer_email === 'string' ? body.customer_email.trim() || null : null,
    ...
    preferred_time_start:
      typeof body?.preferred_time_start === 'string' ? body.preferred_time_start : null,
    preferred_time_end:
      typeof body?.preferred_time_end === 'string' ? body.preferred_time_end : null,
  ...
  if (!payload.location_id || !payload.customer_name || !payload.customer_phone) {
    return NextResponse.json(
      { error: 'location_id, customer_name, and customer_phone are required' },
      { status: 400 },
    )
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (typeof body?.linked_booking_id === 'string' || body?.linked_booking_id === null)
    updates.linked_booking_id = body.linked_booking_id
