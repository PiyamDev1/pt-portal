...
import {
  ...
  getPackageRefundAdjustmentTotal,
...
function autoReservationRows({
  ...
  combination.linkedFlightSelections.forEach((selection) => {
    ...
    rows.push({
      ...baseRow,
      reservation_type: 'flight',
      title: `${familyLabel ? `${familyLabel} - ` : ''}Flight leg - ${selection.group.routeLabel || selection.option.airlineName}`,
      sold_price_total: sold,
      internal_notes: [
  ...
  if (combination.transportOption) {
    ...
    rows.push({
      ...
      title: sharedGroupTransportAllocation
        ? `${familyLabel ? `${familyLabel} - ` : ''}Shared group transport`
        : `${familyLabel ? `${familyLabel} - ` : ''}Transport - ${combination.transportOption.title || 'Selected transport'}`,
      booked_cost_total:
...
function sharedGroupTransportPhysicalRow({
  ...
  const sourceEntry = entries.find(
    (entry) => entry.quote.selected_option?.combination.transportOption,
  )
  const option = sourceEntry?.quote.selected_option?.combination.transportOption
  if (!sourceEntry || !option) return null
  ...
  return {
    ...
    status: 'reservation_pending',
    supplier_name: option.transportMainSupplierName || null,
    currency: sourceEntry.quote.selected_option!.combination.currency,
    ...
    metadata: {
      autoGenerated: true,
      source: 'final_quote_group_transport',
      sourceKey: `shared-group-transport-${option.id}`,
      ...
      transportNetCost,
      transportNetCurrency: option.transportNetCurrency || sourceEntry.quote.currency,
      familyAllocations: entries.map((entry) => ({
        ...
        soldPrice: optionTotal(
          entry.quote.selected_option!.combination.transportOption,
          entry.payload.adults +
            entry.payload.childrenPaying +
            entry.payload.childrenFree +
            entry.payload.infants,
        ),
      })),
    },
  }
...
function autoPaymentRows({
  ...
  const refundAdjustmentRows: GroupTravelPackagePaymentInsert[] = payload.limitedTimeOffers
    .filter(
      (offer) =>
        isLimitedTimeOfferActive(offer) &&
        getPackageQuoteDiscountType(offer) === 'refund_adjustment',
    )
    .flatMap((offer) => {
      const amount = getPackageOfferDiscountTotal(offer, payload)
      if (amount <= 0) return []
      return [
        {
          package_id: packageId,
          ...
          requested_at: now,
          received_at: now,
          receipt_reference: offer.reference || offer.title || quote.title,
          notes: offer.summary || 'Credit transferred from a previous package refund.',
          metadata: {
            autoGenerated: true,
  ...
  if (selection.paymentIntent === 'installment_request') return refundAdjustmentRows
  ...
  if (!breakdown) return refundAdjustmentRows
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (groupContext) {
    ...
    if (sharedTransportRow) reservationRows.push(sharedTransportRow)
  }
