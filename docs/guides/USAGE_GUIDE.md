...
Settings includes the browser-local Mobile/Desktop testing control for every signed-in employee. Depending on role, it can also include staff/department/location hierarchy, booking configuration, service pricing, notice board, account security, physical timeclock devices, receipt metrics, maintenance, document storage, issue reports, and Super Admin server controls.
