...
The root layout selects presentation from the request operating system before rendering. Android and iOS/iPadOS receive `data-device-layout="mobile"` and an initial 430 CSS-pixel app canvas; Windows, macOS, Linux, ChromeOS, and unknown desktop clients receive `data-device-layout="desktop"` with `width=device-width`. `DeviceLayoutSynchronizer` repeats the same OS classification from browser-owned values after hydration, covering proxies that supply generic request headers. It then reconciles the mobile viewport against the device's shorter screen dimension (320–480 CSS pixels). A bounded `zoom` compensation is enabled only when an Android desktop-site/PWA engine refuses that viewport update and retains a canvas at least 25% wider than the mobile target.

Mobile composition is also independent of width media queries. Rules scoped to `html[data-device-layout="mobile"] .dashboard-mobile-shell` restore mobile-only controls, suppress desktop-only controls, keep breakpoint grids single-column, and retain stacked flex layouts even if the browser activates Tailwind `sm`/`md`/`lg`/`xl` utilities. Page-level classes provide the denser Settings tab rail, Timeclock camera/actions, Bookings agenda, and Applications card layout. Desktop selectors and render paths are unchanged. The `pt_portal_device_layout` cookie is an intentional Settings-only testing override and takes priority on both server and client until **Use device default** clears it.

OS-selected visibility utilities (`platform-mobile-*` and `platform-desktop-*`) control deliberately separate login and dashboard compositions as well as the header, notice presentation, safe-area spacing, touch sizing, and bottom navigation. Mobile authentication uses a full-width surface with 56-pixel controls; the dashboard presents modules in a two-column tile grid and never renders the desktop notice rail. The five-slot phone menu resolves three employee-selected module IDs from Supabase auth metadata through `lib/mobileNavigation.ts`, places Home in the centre, and fixes Settings at the far right. The shared header orders the logo, company/branch identity, and parent-directory action before the account/menu controls. `lib/deviceLayout.ts` remains the single OS-classification and preference-parsing source, while `lib/deviceViewport.ts` owns mobile canvas sizing and the wide-viewport fallback. Do not introduce page-local width checks or independent user-agent patterns.
