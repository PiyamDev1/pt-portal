...
import { customerAvailability } from '@/lib/customerPortal/appointments'
import {
...
export const POST = withCustomerIntegrationRoute(async (request) => {
  ...
  const slots = await customerAvailability({
    servicePublicId: input.serviceId,
    branchPublicId: input.branchId,
    date: input.date,
    groupSize: input.groupSize,
  })
  return customerIntegrationOk(slots, context.requestId)
})
