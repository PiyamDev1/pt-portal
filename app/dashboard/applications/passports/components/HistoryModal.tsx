  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Status timeline for ${trackingNumber}`}
        className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto"
        tabIndex={-1}
      >
        <h3 className="font-bold text-xl text-slate-800 mb-1">Status Timeline</h3>
        <p className="text-sm text-slate-500 mb-6">
          Tracking: <span className="font-mono font-bold text-slate-700">{trackingNumber}</span>
        </p>

        {statusHistory?.length === 0 ? (
          <p className="text-center text-slate-400 py-8" role="status" aria-live="polite">
          ...
          <div className="relative">
            ...
            <div className="space-y-6">
              {statusHistory.map((entry, idx: number) => (
                <div key={entry.id || idx} className="relative pl-16">
                  <div
                    className={`absolute left-4 top-1 w-5 h-5 rounded-full border-4 border-white shadow-md ${idx === 0 ? 'bg-green-500' : 'bg-blue-400'}`}
                  ></div>
        ...
        <button
          onClick={onClose}
          className="w-full mt-6 py-3 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-900 transition"
          type="button"
