  ...
  return (
    ...
    >
      ...
      {sharePath && !isCurrent && (
        <a
          href={sharePath}
          className="mt-3 inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg bg-cyan-900 px-3 text-xs font-black text-white transition hover:bg-cyan-950 sm:w-auto"
        >
          View this quote
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
                        Select a family quote below. Your current quote stays marked clearly.
                      </p>
                    </div>
                    <div className="flex gap-2 overflow-x-auto pb-1 lg:max-w-2xl lg:flex-wrap lg:justify-end lg:overflow-visible lg:pb-0">
                      {linkedFamilyCards.map(({ family, index, key, pricing }) => {
                        const label = getLinkedFamilyLabel(family, index)
                        const content = (
                          <>
                            <span className="block truncate">{family.familyLabel || label}</span>
                        ...
                        return family.sharePath && !family.isCurrent ? (
                          <a
                            ...
                            className="min-w-[9rem] shrink-0 rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-black text-cyan-950 transition hover:bg-cyan-100"
                          >
                            {content}
                          </a>
                        ) : (
                          ...
                          >
                            {content}
                            <span className="mt-1 inline-flex rounded-full bg-white/15 px-2 py-0.5 text-[10px] uppercase">
                              Current
                            </span>
                          </button>
                        )
