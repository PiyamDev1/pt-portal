      ...
      <table className="w-full text-sm border-collapse print:text-xs">
        <thead className="sticky top-0 z-10 bg-slate-100 print:static print:bg-slate-200">
          <tr className="border-b-2 border-slate-900">
            <th
              scope="col"
              className="p-3 text-left text-xs font-bold text-slate-700 uppercase print:p-2 print:text-[9px]"
            >
              Date
            </th>
            <th
            ...
            <th
              scope="col"
              className="p-3 text-right text-xs font-bold text-red-700 uppercase print:p-2 print:text-[9px]"
            >
              Debit
            </th>
            <th
              scope="col"
              className="p-3 text-right text-xs font-bold text-green-700 uppercase print:p-2 print:text-[9px]"
            >
              Credit
            </th>
          </tr>
        ...
        <tbody>
          {filteredTransactions.length > 0 ? (
              ...
              rows.push(
                ...
                >
                  ...
                  <td className="p-3 print:p-2">
                    <span
                      className={`px-2 py-0.5 rounded text-[11px] font-bold print:px-1 print:py-0 print:text-[8px] ${
                        isService
                  ...
                  <td className="p-3 text-slate-600 print:p-2 print:text-xs">
                    <div>{tx.remark || '-'}</div>
                    {isService && (
                      <div className="text-xs text-slate-500 print:text-[7px]">
                        Service Plan: {tx.id.substring(0, 8)}
                      </div>
                    )}
                    {tx.loan_payment_methods?.name && (
                      <div className="text-xs text-slate-400 print:text-[7px]">
                        ({tx.loan_payment_methods.name})
                      </div>
                    )}
                  </td>
              ...
              if (isService) {
                ...
                if (installments.length > 0) {
                  ...
                  installments.forEach((installment: InstallmentPayment, idx: number) => {
                    ...
                    rows.push(
                      <tr
                        ...
                        className="border-b border-slate-200 bg-blue-50 hover:bg-blue-100 print:bg-blue-50 print:hover:bg-blue-50"
                      >
                        <td className="p-3 text-slate-600 print:p-2 print:text-xs">
                        ...
                        <td className="p-3 print:p-2">
                          <span
                            className={`px-2 py-0.5 rounded text-[11px] font-bold print:px-1 print:py-0 print:text-[8px] ${statusColor}`}
                          >
                        ...
                        <td className="p-3 text-slate-600 text-sm print:p-2 print:text-xs">
                          ...
                          <div className="text-xs text-slate-500 print:text-[7px]">
                            Plan: {tx.id.substring(0, 8)} | ID: {installment.id.substring(0, 8)}
                          </div>
                        </td>
                        ...
                        <td className="p-3 text-right font-mono text-slate-400 print:p-2 print:text-xs">
                          {parseAmount(installment.amount_paid) > 0
                            ? `£${parseAmount(installment.amount_paid).toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                            : '-'}
                        </td>
                      </tr>,
                    )
                  ...
                  if (serviceLoan && serviceLoan.term_months) {
                    ...
                    scheduleRows.forEach((installment, idx: number) => {
                      rows.push(
                        <tr
                          key={`install-${i}-${idx}`}
                          className="border-b border-slate-200 bg-blue-50 hover:bg-blue-100 print:bg-blue-50 print:hover:bg-blue-50"
                        >
                          ...
                          <td className="p-3 text-slate-600 text-sm print:p-2 print:text-xs">
                            <div>
                              Total £{txAmount.toFixed(2)}, Remaining £
                              {installment.remaining.toFixed(2)}
                            </div>
                            <div className="text-xs text-slate-500 print:text-[7px]">
                              Term {idx + 1}/{serviceLoan.term_months} | Plan:{' '}
                              {tx.id.substring(0, 8)}
                            </div>
                          </td>
            ...
            <tr>
              <td
                colSpan={5}
                className="p-6 text-center text-slate-400 print:p-3 print:text-xs"
                role="status"
                aria-live="polite"
              >
