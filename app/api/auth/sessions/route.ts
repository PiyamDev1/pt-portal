...
export async function GET(request: Request) {
  try {
    ...
    if (dbError) {
      ...
      return apiOk({ sessions: formattedSessions })
    }
    ...
    return apiError(toErrorMessage(error, 'Session fetch error'), 500)
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return apiError(toErrorMessage(error, 'Session error'), 500)
  }
}
