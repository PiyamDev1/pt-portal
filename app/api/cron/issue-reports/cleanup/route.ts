...
export async function GET(request: Request) {
  ...
  try {
    ...
    const now = new Date()
    const artifactPurgeBefore = new Date(
      now.getTime() - ARTIFACT_RETENTION_DAYS * 24 * 60 * 60 * 1000,
    ).toISOString()
    const ticketDeleteBefore = new Date(
      now.getTime() - TICKET_RETENTION_DAYS * 24 * 60 * 60 * 1000,
    ).toISOString()
    ...
    if (artifactReportIds.length > 0) {
      ...
      for (const artifact of (artifacts || []) as Array<{
        ...
        try {
          ...
          // Best effort cleanup: continue even if one artifact delete fails.
        }
      }
    }
    ...
    if (ticketIdsToDelete.length > 0) {
      ...
      for (const artifact of (remainingArtifacts || []) as Array<{
        id: string
        storage_bucket: string
        storage_key: string
      }>) {
        try {
          ...
          // Best effort cleanup: continue ticket deletion regardless of artifact removal failures.
        }
      }

      const { error: deleteError } = await issueReportsTable.delete().in('id', ticketIdsToDelete)
    ...
    return apiError(toErrorMessage(error, 'Issue report cleanup failed'), 500)
  }
}
