...
import {
  ...
  startConsoleCapture,
} from './issue-reporter/consoleCapture'
  ...
  const captureScreenshot = async () => {
    ...
    const canvas = await html2canvas(document.body, {
      ...
      onclone: (documentClone) => {
        documentClone
          .querySelectorAll(
            'input[type="password"], textarea[data-issue-report-redact="true"], input[data-issue-report-redact="true"]',
          )
          .forEach((element) => {
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-3 text-xl font-bold text-slate-900">
                  Send fault details to Master Admin
                </h2>
            ...
            {submittedTicketId ? (
              <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
                ...
                <p className="mt-1 text-sm">
                  Reference ID: <span className="font-mono">{submittedTicketId}</span>
                </p>
                <button
                  type="button"
                  onClick={handleClose}
              ...
              <div className="mt-5 space-y-4">
                ...
                <div className="grid gap-3">
                  <label className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
                    ...
                    <span>
                      <span className="flex items-center gap-2 font-semibold text-slate-900">
                        <Camera className="h-4 w-4" /> Include screenshot
                      </span>
                      <span className="mt-1 block text-xs text-slate-500">
                        Captures the current visible page only.
                      </span>
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
                    ...
                    <span>
                      <span className="flex items-center gap-2 font-semibold text-slate-900">
                        <TerminalSquare className="h-4 w-4" /> Include browser console
                      </span>
                      <span className="mt-1 block text-xs text-slate-500">
                        Recent entries queued: {consoleCount}
                      </span>
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
                    ...
                    <span>
                      <span className="flex items-center gap-2 font-semibold text-slate-900">
                        <Bug className="h-4 w-4" /> Include failed API requests
                      </span>
                      <span className="mt-1 block text-xs text-slate-500">
                        Recent failed requests queued: {failedRequestCount}
                      </span>
                    </span>
                  </label>
                </div>
