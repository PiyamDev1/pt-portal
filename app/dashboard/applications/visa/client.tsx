...
import type {
  SaveVisaApplicationPayload,
  VisaApplicationRecord,
  VisaApplicationsClientProps,
  VisaFormState,
  VisaMetadata,
} from '@/app/types/visa'
...
export default function VisaApplicationsClient({
  ...
  const handleSave = async (data: VisaFormState) => {
    try {
      const payload: SaveVisaApplicationPayload = {
        ...data,
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={VISA_TABLE_COLUMNS.length + 1}
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-xs text-slate-400 font-mono">
                        {item.passport_number_used}
                      </div>
                    </td>
                    {/* Nationality */}
                    <td className="p-4 text-sm text-slate-600">
                      {item.applicants?.nationality || '-'}
                    </td>
                    {/* Country */}
                    <td className="p-4 text-sm text-slate-600">
                    ...
                    <td className="p-4">
                      {(() => {
                        ...
                        return (
                          <span
                            className={`text-xs font-semibold px-2 py-1 rounded border ${colors}`}
                          >
                            {item.status}
                          </span>
                        )
