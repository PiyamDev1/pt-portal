...
export default function NewApplicationForm({ formData, isSubmitting, onChange, onBlur, onSubmit, errors = {}, metadata }: Props) {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
           ...
           <div className="grid grid-cols-2 gap-3">
             <select name="applicationType" onChange={onChange} className="p-2 bg-white border border-green-100 rounded-lg text-sm" value={formData.applicationType}>
               {metadata.applicationTypes.map((type) => (
                 <option key={type} value={type}>{type}</option>
               ))}
             </select>
             <select name="speed" onChange={onChange} className="p-2 bg-white border border-green-100 rounded-lg text-sm font-bold text-slate-700" value={formData.speed}>
               {metadata.speeds.map((speed) => (
                 <option key={speed} value={speed}>{speed}</option>
               ))}
             </select>
             <select name="category" onChange={onChange} className="p-2 bg-white border border-green-100 rounded-lg text-sm" value={formData.category}>
               {metadata.categories.map((cat) => (
                 <option key={cat} value={cat}>{cat}</option>
               ))}
             </select>
             <select name="pageCount" onChange={onChange} className="p-2 bg-white border border-green-100 rounded-lg text-sm" value={formData.pageCount}>
