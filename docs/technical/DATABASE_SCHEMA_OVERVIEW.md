...
| `20260902_ticketing_maintenance_admin_operations.sql`           | audited Maintenance Admin team-ledger viewing, on-behalf entry/completion, itinerary/staff/sale correction, while archive/delete remains admin-only                                                                                    |
| `20260902_ticketing_admin_date_corrections.sql`                 | optimistic, idempotent booking/issued/deadline correction with a private trigger context, immutable audit, superseding Commission source facts, and capability `2026090203`                                                            |
| `20260902_ticketing_correction_refund_hardening.sql`            | forward-only staff/commercial, Refund, and date-correction hardening; locked replay authorization, strict privileged JSON validation, unambiguous branch-local deadlines, confirmation withdrawal, and capability `2026090204`         |
...
| `20260829_commission_staff_profiles_assistance_scope.sql`       | all-or-selected primary-agent scope for employee-owned Ticket Assistance plans, legacy all-agent compatibility, and Commission capability `2026082905`                                                                                 |
...
| `20260830_commission_package_shadow_integration.sql`            | closed-Package source snapshots, correction-safe fixed/passenger/profit commission, historical backfill, and capability `2026083003`                                                                                                   |
| `20260902_commission_package_return_payout.sql`                 | Complete-Checked package handoff, return-date + 3 Commission timing, future-event deferral, and invoice-release-independent reconciliation capability `2026090202`                                                                     |
| `20260903_commission_package_reservation_payment_authority.sql` | Reservations become the sole Package financial/Commission authority; completed Payment-tab movements settle the balance, and optional customer invoices are excluded. Capability `2026090301`                                          |
