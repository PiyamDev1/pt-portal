...
function autoPaymentRows({
  ...
  if (selection.paymentIntent === 'deposit_only') {
    const amount = roundMoney(payload.depositAmount || 0)
    if (amount <= 0) return []
    ...
    return [
      {
        ...
        requested_at: now,
        due_at: now,
        notes: `Customer chose to pay the minimum deposit by ${depositMethodLabel}. Deposit is non-refundable and must be paid as one full deposit payment.`,
        metadata: {
          autoGenerated: true,
