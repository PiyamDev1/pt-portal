/**
 * API Route: Submit Manual Entry Code
 *
 * POST /api/timeclock/manual-entry/submit
 *
 * Validates and consumes a manual-entry code generated by the
 * /generate endpoint to record a timeclock punch event.
 *
 * Validation steps:
 *   1. Verifies the HMAC signature on the payload
 *   2. Checks code has not expired
 *   3. Checks for duplicate punch within DUPLICATE_WINDOW_MS (8 s)
 *   4. Checks for duplicate in-day punch of same type
 *   5. Marks the code as used to prevent replay
 *
 * Request Body: { code: string, deviceId?: string }
 * Response Success (200): { eventId }
 * Response Errors:
 *   400 - Invalid code format, expired, or duplicate punch
