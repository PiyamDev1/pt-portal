...
function LMSClientInner({ currentUserId }: LMSClientProps) {
  ...
  const [showTransaction, setShowTransaction] = useState<
    (Account & { transactionType?: string }) | null
  >(null)
  ...
  const getStatusBadge = useCallback((account: Account) => {
    if (account.balance <= 0) {
      return (
        <span
          className={`px-2 py-0.5 bg-${STATUS_COLORS.SETTLED.bg} ${STATUS_COLORS.SETTLED.text} text-[10px] font-bold rounded border ${STATUS_COLORS.SETTLED.border}`}
        >
          SETTLED
        </span>
      )
    ...
    if (account.isOverdue) {
      return (
        <span
          className={`px-2 py-0.5 bg-${STATUS_COLORS.OVERDUE.bg} ${STATUS_COLORS.OVERDUE.text} text-[10px] font-bold rounded border ${STATUS_COLORS.OVERDUE.border} flex items-center gap-1 w-fit`}
        >
    ...
    if (account.isDueSoon) {
      return (
        <span
          className={`px-2 py-0.5 bg-${STATUS_COLORS.DUE_SOON.bg} ${STATUS_COLORS.DUE_SOON.text} text-[10px] font-bold rounded border ${STATUS_COLORS.DUE_SOON.border} flex items-center gap-1 w-fit`}
        >
    ...
    return (
      <span
        className={`px-2 py-0.5 bg-${STATUS_COLORS.ACTIVE.bg} ${STATUS_COLORS.ACTIVE.text} text-[10px] font-bold rounded border ${STATUS_COLORS.ACTIVE.border}`}
      >
        ACTIVE
      </span>
    )
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th
                    scope="col"
                    className="p-3 text-left text-xs font-bold text-slate-600 uppercase"
                  >
                    Customer
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    className="p-3 text-center text-xs font-bold text-slate-600 uppercase"
                  >
                    Status
                  </th>
                  <th
                    scope="col"
                    className="p-3 text-right text-xs font-bold text-slate-600 uppercase"
                  >
                    Balance
                  </th>
                  <th
