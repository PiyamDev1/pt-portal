...
function autoReservationRows({
  ...
  return buildPackageQuoteReservationDrafts({
    payload,
    combination,
    familyLabel,
    ...
    metadata: {
      ...
      quoteSoldPrice: draft.soldPriceTotal,
      quoteDiscountTotal: draft.discountTotal,
      quoteInternalNotes: draft.internalNotes,
      quoteSyncedAt: now,
    },
  }))
}
...
function sharedGroupTransportPhysicalRow({
  ...
  const draft = buildSharedGroupTransportDraft(
    entries.map((entry) => ({
  ...
  return {
    ...
    status: 'reservation_pending',
    supplier_name:
      typeof draft.metadata.transportMainSupplierName === 'string'
        ? draft.metadata.transportMainSupplierName || null
        : null,
...
function autoPaymentRows({
  ...
  const refundAdjustmentRows: GroupTravelPackagePaymentInsert[] = combination.appliedOffers
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (groupContext) {
    const { error: groupMemberUpdateError } = await supabase
      .from('travel_package_group_members')
      .update({ package_id: packageFolder.id })
      .eq('group_id', groupContext.group.id)
    if (groupMemberUpdateError) {
      await removeIncompleteGroupPackage()
      return apiError(
        `${groupMemberUpdateError.message}. Run scripts/migrations/2026082802_repair_combined_group_package_links.sql before converting a combined group.`,
        500,
      )
    }
