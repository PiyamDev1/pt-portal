      ...
      const windows = [
        {
          ...
          hours: Math.min(168, Math.max(1, Number(setting.reminder_hours_before) || 24)),
        },
        {
          key: 'same_day',
          enabled: setting.same_day_reminder_enabled !== false,
          hours: Math.min(12, Math.max(1, Number(setting.same_day_reminder_hours_before) || 2)),
        },
      ]

      for (const reminderWindow of windows) {
        ...
        for (const booking of candidates) {
          ...
          if (reminderWindow.key === 'advance' && existing?.reminder_sent_at) continue
          if (reminderWindow.key === 'same_day' && existing?.same_day_reminder_sent_at) continue
          ...
          const upsertPayload = {
            ...
            metadata: {
              last_reminder_kind: reminderWindow.key,
          ...
          await storeBookingEmailAttempt(supabase, {
            ...
            metadata: {
              reminder_kind: reminderWindow.key,
              reminder_hours_before: reminderWindow.hours,
            },
          })
