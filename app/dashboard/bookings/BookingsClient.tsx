...
export default function BookingsClient({
  ...
  const [savedViews, setSavedViews] = useState<SavedBookingView[]>([])
  ...
  const [notesAutosaveState, setNotesAutosaveState] = useState<
  ...
  const fetchWaitlist = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/bookings/waitlist?location_id=${encodeURIComponent(selectedLocationId)}`,
        { cache: 'no-store' },
      )
      const json = await res.json()
      if (!res.ok) return
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const res = await fetch(
          `/api/bookings/preferences?location_id=${encodeURIComponent(selectedLocationId)}`,
          { cache: 'no-store' },
        )
        const json = await res.json()
  ...
  const loadSlotsFor = useCallback(
    async (date: string, serviceId: string, personCount: number): Promise<SlotLoadResult> => {
    ...
    (options?: {
      ...
      if (requestedStartTime) {
        ...
        if (
          ...
          requestedStartDate.toISOString().slice(0, 10) !== safeDate
        ) {
          safeStartTime = ''
        }
      }
  ...
  const trackBookingMetric = useCallback(
  ...
  const persistSavedViews = useCallback(
    async (nextSavedViews: typeof savedViews) => {
  ...
  const resendBookingEmail = useCallback(
    async (
  ...
  const updateStatus = async (
    id: string,
    status: string,
    options?: { skipConfirm?: boolean; skipUndo?: boolean },
  ) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409) {
          toast.error(
            json.error || 'Appointment changed by another staff member. Reload and try again.',
          )
          await fetchBookings(false)
        ...
        void trackBookingMetric('booking_status_error', {
          bookingId: id,
          nextStatus: status,
      ...
      setBookings((prev) =>
        prev.map((b) =>
          b.id === id ? { ...b, ...json.booking, status: status as BookingStatus } : b,
        ),
      )
    ...
    async (
      ...
      if (!options?.skipConfirm) {
        const confirmed = await confirmWithToast(
          'Reschedule this appointment to the new time?',
          'Reschedule',
        )
        if (!confirmed) return
      }
      ...
      setUpdatingId(id)
      // Optimistic update
      setBookings((prev) => prev.map((b) => (b.id !== id ? b : { ...b, start_time: newStartTime })))
      try {
        const res = await fetch(`/api/bookings/${id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) {
          ...
          void trackBookingMetric('booking_reschedule_error', {
            bookingId: id,
            statusCode: res.status,
          })
  ...
  const flagNoShow = useCallback(
    async (booking: BookingWithService) => {
      const confirmed = await confirmWithToast(
        'Flag this appointment as a no-show and apply penalty tracking?',
        'Flag no-show',
      )
      if (!confirmed) return
  ...
  useEffect(() => {
    ...
    if (!appointmentForm.manual_override || !appointmentForm.date || !appointmentForm.start_time)
      return
    ...
    const effectiveDuration =
      ...
      getServicePersonUnits(selectedService, appointmentForm.person_count) *
    ...
    notesAutosaveTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (res.ok) {
          setBookings((prev) =>
            prev.map((booking) =>
              booking.id === editingBooking.id ? { ...booking, notes: trimmed } : booking,
            ),
          )
  ...
  const saveAppointment = async () => {
    ...
    const manualStartIso =
      appointmentForm.manual_override && appointmentForm.date && appointmentForm.start_time
        ? new Date(`${appointmentForm.date}T${appointmentForm.start_time}:00`).toISOString()
        : ''
    const manualEndIso =
      appointmentForm.manual_override && appointmentForm.date && appointmentForm.end_time
    ...
    if (
      ...
      !appointmentForm.phone_local ||
      !appointmentForm.service_id ||
      (!appointmentForm.manual_override && !appointmentForm.start_time)
    ) {
    ...
    try {
      const customer_phone =
        `${appointmentForm.phone_country_code} ${normalizeLocalPhone(appointmentForm.phone_local)}`.trim()
      ...
      if (editingBooking) {
        const confirmed = await confirmWithToast(
          'Apply these amendments to this appointment?',
          'Apply',
        )
        if (!confirmed) return
      }
      ...
      if (editingBooking) {
        const res = await fetch(`/api/bookings/${editingBooking.id}`, {
          ...
          body: JSON.stringify({
            ...
            tags: appointmentForm.tags
              .split(',')
              .map((tag) => tag.trim())
              .filter(Boolean),
            start_time: appointmentForm.manual_override
              ? manualStartIso
              : appointmentForm.start_time,
        ...
        if (!res.ok) {
          ...
          toast.error(json.error || 'Failed to update appointment')
          void trackBookingMetric('booking_amend_error', {
        ...
        if (!res.ok) {
          ...
          void trackBookingMetric('booking_create_error', {
            statusCode: res.status,
            manual_override: appointmentForm.manual_override,
          })
          return
        ...
        void trackBookingMetric('booking_created', {
          manual_override: appointmentForm.manual_override,
        })
      }
  ...
  const invalidLocalPhone =
    appointmentForm.phone_local.length > 0 && !isValidLocalPhone(appointmentForm.phone_local)
  const selectedManualService = serviceOptions.find(
    (service) => service.id === appointmentForm.service_id,
  )
  const expectedManualDuration = selectedManualService
    ? selectedManualService.duration_minutes +
      getServicePersonUnits(selectedManualService, appointmentForm.person_count) *
        selectedManualService.duration_per_additional_person_minutes
    : null
  const manualDurationMinutes =
    appointmentForm.manual_override && appointmentForm.start_time && appointmentForm.end_time
              ...
              >
                <div className="grid grid-cols-7 border-b border-slate-200 bg-[linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)]">
                  {CALENDAR_DAY_LABELS.map((label) => (
                    <div
                      key={label}
                      className="px-3 py-2 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center"
                    >
                      {label}
                    </div>
                  ))}
                      ...
                      >
                        ...
                        <div className="mt-3 space-y-1.5">
                          {dayBookings.slice(0, 3).map((booking) => (
                            <div
                              key={booking.id}
                              className="flex items-center gap-1.5 rounded-md bg-slate-50/80 px-1.5 py-1 transition-colors hover:bg-slate-100/80"
                            >
                              <span
                                className={`h-1.5 w-1.5 rounded-full ${statusDotClass(booking.status)}`}
                              />
              ...
              {loading ? (
                ...
                <div className="py-14 text-center">
                  <div className="mx-auto max-w-sm rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-8">
                    ...
                    <p className="mt-2 text-sm text-slate-400">
                      Try another week, change the source filter, or add a new appointment.
                    </p>
                  </div>
                </div>
              ) : (
                weekDays.map((day, index) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`px-4 sm:px-5 py-3 border-b border-slate-100 flex items-center gap-2 ${isToday ? 'bg-indigo-50' : 'bg-slate-50'}`}
                      >
                        <span
                          className={`text-sm font-semibold ${isToday ? 'text-indigo-700' : 'text-slate-600'}`}
                        >
                          {formatDateLabel(day)}
                        </span>
                        {isToday && (
        ...
        {!showSettings && cancelledBookings.length > 0 && (
          <div className="overflow-hidden rounded-2xl border border-red-200 bg-white shadow-sm">
            <div className="px-5 py-3 border-b border-red-100 bg-[linear-gradient(180deg,_#fff1f2_0%,_#fef2f2_100%)] flex items-center justify-between">
              <h3 className="inline-flex items-center gap-1.5 text-sm font-semibold text-red-700">
                <CloseIcon className="h-4 w-4" />
                Cancelled Appointment Logs
              </h3>
            ...
            <div className="divide-y divide-red-50 max-h-72 overflow-y-auto">
              {cancelledBookings
                ...
                .map((booking) => (
                  <div
                    key={booking.id}
                    className="px-5 py-3 flex items-center justify-between gap-3 hover:bg-red-50/40 transition-colors"
                  >
                    <div>
                      <p className="text-sm font-medium text-slate-700">
                        {formatDateLabel(new Date(booking.start_time))}{' '}
                        {formatTime(booking.start_time)} · {booking.customer_name}
                      </p>
                ...
                <div key={entry.id} className="flex items-center justify-between gap-3 px-5 py-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">
                      {entry.preferred_date || 'Flexible date'}{' '}
          ...
          <div className="max-h-[94vh] w-full max-w-3xl space-y-4 overflow-y-auto rounded-b-none rounded-t-[24px] border border-white/70 bg-white p-4 shadow-[0_28px_90px_-36px_rgba(15,23,42,0.5)] transition-all duration-200 md:max-h-[90vh] md:rounded-[28px] md:p-5">
            <div className="sticky top-0 z-20 -mx-4 -mt-4 flex items-center justify-between border-b border-slate-100 bg-white/95 px-4 py-3 backdrop-blur md:static md:m-0 md:bg-transparent md:p-0 md:pb-3">
              <div>
                ...
                {!editingBooking && (
                  <p className="mt-1 text-xs text-slate-500">Draft status: {draftState}</p>
                )}
              </div>
              <button
                onClick={closeAppointmentModal}
                className="ui-tap ui-focus rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-700"
              >
                Close
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {!showRescheduleOnly && (
                <label className="text-sm text-slate-700">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAppointmentForm((p) => ({ ...p, customer_name: e.target.value }))
                    }
              ...
              {!showRescheduleOnly && (
                <label className="text-sm text-slate-700">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAppointmentForm((p) => ({ ...p, customer_email: e.target.value }))
                    }
                    className="mt-1 w-full border border-slate-300 rounded px-3 py-2"
                  />
                </label>
              ...
              {!showRescheduleOnly && (
                <label className="text-sm text-slate-700">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAppointmentForm((p) => ({ ...p, phone_country_code: e.target.value }))
                    }
                    className="mt-1 w-full border border-slate-300 rounded px-3 py-2"
                  >
              ...
              {!showRescheduleOnly && (
                <label className="text-sm text-slate-700">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAppointmentForm((p) => ({ ...p, phone_local: e.target.value }))
                    }
                    className="mt-1 w-full border border-slate-300 rounded px-3 py-2"
                  />
                  {invalidLocalPhone && (
              ...
              <label className="text-sm text-slate-700">
                ...
                <input
                  type="date"
                  min={editingBooking ? undefined : todayDateKey}
                  value={appointmentForm.date}
                  onChange={(e) =>
                    setAppointmentForm((p) => ({
                      ...p,
                      date: e.target.value,
                      start_time: p.manual_override ? p.start_time : '',
                    }))
                  }
                  className="mt-1 w-full border border-slate-300 rounded px-3 py-2"
                />
              ...
              {appointmentForm.manual_override && (
                <>
                  <div className="md:col-span-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                    Manual override uses your local browser time zone. Set the exact start and end
                    time you want saved.
                  </div>
                  ...
                  <label className="text-sm text-slate-700">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAppointmentForm((p) => ({ ...p, start_time: e.target.value }))
                      }
                      className="mt-1 w-full border border-slate-300 rounded px-3 py-2"
                    />
                  </label>
                  ...
                  <label className="text-sm text-slate-700">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAppointmentForm((p) => ({ ...p, end_time: e.target.value }))
                      }
                      className="mt-1 w-full border border-slate-300 rounded px-3 py-2"
                    />
                    ...
                    <label className="mt-2 block text-sm text-slate-700">
                      ...
                      {editingBooking && (
                        <p className="mt-1 text-xs text-slate-500">
                          ...
                          {notesAutosaveState === 'error' &&
                            ' Save failed. Please click Save Changes.'}
                        </p>
                      )}
                    </label>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  {selectedService && (
                    <div className="text-xs text-slate-600 rounded border border-slate-200 bg-slate-50 px-3 py-2">
                      <p>
                        Effective duration:{' '}
                        <span className="font-semibold">{effectiveDuration} min</span>
                      </p>
                      <p className="mt-1">
                        Buffer:{' '}
                        <span className="font-semibold">{selectedService.buffer_minutes} min</span>
                      </p>
                      <p className="mt-1">
                        Next appointment gap:{' '}
                        <span className="font-semibold">{nextStartGap} min</span>
                      </p>
                    </div>
                  )}
                </div>
              ...
              (() => {
                ...
                const modalDuration = svc
                  ? svc.duration_minutes +
                    getServicePersonUnits(svc, appointmentForm.person_count) *
                      svc.duration_per_additional_person_minutes
                  : 30
                const dateBookings = allActiveBookings.filter((b) =>
                  b.start_time.startsWith(appointmentForm.date),
                )
                return (
                  <div className="text-sm text-slate-700">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-400">
                          Click the timeline to pick a start time
                        </span>
                      </div>
                    </div>
                    {loadingSlots ? (
                      ...
                      <div className="h-80 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center text-slate-400 text-sm">
                        Select a service and date to see the timeline.
                      </div>
                    ) : (
                      <div className="space-y-3">
                        <SlotTimeline
                    ...
                    {appointmentForm.start_time && (
                      <p className="mt-2 text-xs text-indigo-700 font-medium">
                        Selected: {formatTime(appointmentForm.start_time)} (duration:{' '}
                        {modalDuration} min)
                      </p>
                    )}
                    {editingBooking &&
                      ...
                      appointmentForm.date &&
                      appointmentForm.service_id &&
                      appointmentForm.start_time === editingBooking.start_time && (
                        <p className="mt-1 text-xs text-emerald-700">
                          Current booked time retained.
                        </p>
                      )}
                    {editingBooking && (
                      <p className="mt-1 text-xs text-slate-500">
                        Tip: use the quick reschedule chips above or click a new time on the
                        timeline.
                      </p>
                    )}
                  </div>
              ...
              <div className="space-y-3">
                <div className="text-sm text-slate-700 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3">
                  ...
                  <p className="mt-1 text-xs text-indigo-600">
                    Existing appointments for this date are shown below to avoid accidental double
                    booking.
                  </p>
                </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-white">
                  ...
                  {!appointmentForm.date ? (
                    <div className="px-3 py-3 text-xs text-slate-500">
                      Pick a date to view existing appointments.
                    </div>
                  ) : (
                    (() => {
                      ...
                      if (existing.length === 0) {
                        return (
                          <div className="px-3 py-3 text-xs text-emerald-700">
                            No existing appointments for this date.
                          </div>
                        )
                      }
                          ...
                          {existing.map((booking) => (
                            ...
                            >
                              ...
                              <p className="mt-0.5 text-[11px] text-slate-500">
                                {booking.booking_services?.name || 'Service'} · {booking.status}
                              </p>
                            </button>
                          ))}
                        </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="sticky bottom-0 z-20 -mx-4 -mb-4 flex flex-col gap-2 border-t border-slate-100 bg-white/95 px-4 py-3 backdrop-blur md:static md:m-0 md:flex-row md:items-center md:justify-end md:border-t-0 md:bg-transparent md:p-0">
                ...
                {editingBooking &&
                  (editingBooking.status === BookingStatus.PENDING ||
                    editingBooking.status === BookingStatus.CONFIRMED) &&
                  !cancelConfirmOpen && (
                    <button
                ...
                <button
                  onClick={closeAppointmentModal}
                  className="ui-tap ui-focus inline-flex min-h-11 w-full items-center justify-center gap-1.5 rounded border border-slate-300 px-4 py-2 text-sm md:min-h-0 md:w-auto"
                >
                  <CloseIcon className="h-4 w-4" />
                  Close
                </button>
                <button
                  ...
                  disabled={savingBooking || invalidLocalPhone}
                  className="ui-tap ui-focus inline-flex min-h-11 w-full items-center justify-center gap-1.5 rounded bg-indigo-600 px-4 py-2 text-sm text-white disabled:opacity-50 md:min-h-0 md:w-auto"
                >
                ...
                {editingBooking && editingBooking.status !== BookingStatus.CANCELLED && (
                  <button
                    onClick={() => void flagNoShow(editingBooking)}
                    disabled={updatingId === editingBooking.id}
                    className="ui-tap ui-focus inline-flex min-h-11 w-full items-center justify-center gap-1.5 rounded border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700 disabled:opacity-50 md:min-h-0 md:w-auto"
                  >
                    Flag No-Show
                  </button>
                )}
