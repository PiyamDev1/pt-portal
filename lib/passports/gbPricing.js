...
export function normaliseGbPricingText(value) {
  return String(value || '')
    .trim()
    .toLowerCase()
    .replace(/\b(\d+)\s*(wks?|weeks?)\b/g, '$1 week')
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
}
...
export function gbPricingRowMatches(
  row,
  { ageGroup, pages, serviceType },
  { allowAliases = true } = {},
) {
  const ageMatches = normaliseGbPricingText(row.age_group) === normaliseGbPricingText(ageGroup)
